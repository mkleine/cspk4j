...
public class ConsoleCspSimulator extends SynchronousCspSimulator {
	
	public ConsoleCspSimulator(CspEnvironment environment) {
		super(environment);
	}

	@Override
	protected CspEvent chooseEvent() {
		CspEvent cspEvent = ConsoleUtils.selectValue(Collections.unmodifiableCollection(offeredEvents));
		return cspEvent;
	}
}
